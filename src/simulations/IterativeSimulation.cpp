...
IterativeSimulation::IterativeSimulation(unsigned int id) : id(id), iteration(0), distance(0), maxDistance(0),
                                                            done(false) {}
...
void hp::runHopperProblem(int id) {
  ...
  while (true) {
    ...
    log << "Iteration #" << iterativeSimulation->getIteration() << " at distance " << iterativeSimulation->getDistance()
        << std::endl;
  }
...
void hp::multithreadSimulation(int threadsNum) {
  ...
  std::sort(iterativeSimulations.begin(), iterativeSimulations.end(),
            [](const std::unique_ptr<IterativeSimulation> &a, const std::unique_ptr<IterativeSimulation> &b) {
  ...
  for (std::unique_ptr<IterativeSimulation> &simulation: iterativeSimulations) {
    ...
    std::cout << "Sim ID " << simulation->getID() << " max distance achieved was " << simulation->getMaxDistance()
              << ". Took " << simulation->getIteration() << " iterations" << std::endl;
  }

  std::cout << "Average iterations: " << totalIterations / (double) iterativeSimulations.size()
            << "   Average max distance: " << totalDistance / (double) iterativeSimulations.size() << std::endl;
}